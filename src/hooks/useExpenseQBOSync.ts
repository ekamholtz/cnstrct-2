...
export const useExpenseQBOSync = () => {
  ...
  return useMutation({
    mutationFn: async (expenseData: ExpenseData) => {
      try {
        ...
        if (expenseData.qbo_sync_status === 'synced' && expenseData.qbo_entity_id) {
          toast({
        ...
        const vendorId = await qboService.getVendorIdForExpense(expenseData.vendor_name);
        
        // Step 2: Get QBO entity reference for project
        const projectQBOId = await qboService.getEntityReference('project', expenseData.project_id);
        ...
        const billData = {
          ...
          TxnDate: expenseData.expense_date
        };
        ...
        await qboService.storeEntityReference('expense', expenseData.id, billId);
